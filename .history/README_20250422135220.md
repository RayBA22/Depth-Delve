...
## ⚙️ Difficultés & Contraintes techniques

Le projet a été développé en **C++ bas niveau**, avec un souci de **séparation claire entre l’IHM (interface graphique)** et la **logique du jeu**. Plutôt que d’utiliser des fonctionnalités avancées de **SFML**, j’ai choisi d’implémenter moi-même la plupart des systèmes (rendu, collisions, inventaire...), afin de mieux comprendre les mécanismes internes et garder un contrôle total sur le code.

Ces choix ont apporté plus de flexibilité, mais ont aussi complexifié le développement, notamment en termes de gestion mémoire et d’organisation du projet.
